	...
	for iPlayer in range(0,int(gc.getMAX_PLAYERS())):
		...
		if pPlayer.isAlive() :
			if not map.plot(iPlayer, 0).isCity() :
				...
				map.plot(iPlayer, 0).setTerrainType (CvUtil.findInfoTypeNum(gc.getTerrainInfo, gc.getNumTerrainInfos(), "TERRAIN_ICE"), True, True)
...
def showUnitClass(UnitClass):
	...
	for iPlayer in range(0,int(gc.getMAX_PLAYERS())):
		if (gc.getPlayer(iPlayer).isAlive() and not gc.getPlayer(iPlayer).isNPC()):
