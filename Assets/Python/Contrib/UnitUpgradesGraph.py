	...
	def getGraph(self):
		...
		for k in range(self.getNumberOfUnits()):
			...
			if (self.bUnit == True):
				...
				iPrereq = gc.getUnitInfo(unit).getPrereqAndBonus()
				if (iPrereq >= 0):
					if (gc.getBonusInfo(iPrereq).getBonusClassType() == gc.getInfoTypeForString("BONUSCLASS_CULTURE")):
						continue
				# End ignore cultural units 
		...
		while (len(self.mGraphs) > mGraphIndex):
			...
			map = {}
			map[0] = set([unit])
			#~ for iterlimit in range(40):
			for iterlimit in range(20):
