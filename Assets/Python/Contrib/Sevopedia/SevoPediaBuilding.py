...
#
# Edited by MrAzure 
# Removed Animations, added Replaced By, Strategy Text box,
# Vertically increased Special Abilities Screen, Removed Background textstring from History
# For C2C only modification needed is to have the textstring TXT_KEY_PEDIA_REPLACED_BY somewhere in mod.
...
class SevoPediaBuilding:
	...
	def __init__(self, main):
		...
		self.X_STRATEGY_PANE = self.X_BUILDING_PANE + self.W_BUILDING_PANE + 10
		self.Y_STRATEGY_PANE = 56
		self.W_STRATEGY_PANE = 280
		self.H_STRATEGY_PANE = 124

		self.X_PREREQ_PANE = self.X_BUILDING_PANE
		self.Y_PREREQ_PANE = self.Y_BUILDING_PANE + self.H_BUILDING_PANE + 10
		self.W_PREREQ_PANE = 294
		self.H_PREREQ_PANE = 110

		self.X_REPLACED_BY_PANE = self.X_PREREQ_PANE + self.W_PREREQ_PANE + 10
		self.Y_REPLACED_BY_PANE = self.Y_PREREQ_PANE
		self.W_REPLACED_BY_PANE = 294
		self.H_REPLACED_BY_PANE = 110
...
#		self.SCALE_ANIMATION = 0.7
		...
		self.H_SPECIAL_PANE = 190 + 70

		self.X_HISTORY_PANE = self.X_SPECIAL_PANE
		self.W_HISTORY_PANE = self.W_SPECIAL_PANE
		self.Y_HISTORY_PANE = self.Y_SPECIAL_PANE + self.H_SPECIAL_PANE - 2
		self.H_HISTORY_PANE = self.top.B_PEDIA_PAGE - self.Y_HISTORY_PANE + 20
...
#		screen.addBuildingGraphicGFC(self.top.getNextWidgetName(), self.iBuilding, self.X_BUILDING_ANIMATION, self.Y_BUILDING_ANIMATION, self.W_BUILDING_ANIMATION, self.H_BUILDING_ANIMATION, WidgetTypes.WIDGET_GENERAL, -1, -1, self.X_ROTATION_BUILDING_ANIMATION, self.Z_ROTATION_BUILDING_ANIMATION, self.SCALE_ANIMATION, True)
	...
	def placeStats(self):
		...
		if (isWorldWonderClass(buildingClassType)):
			iMaxInstances = gc.getBuildingClassInfo(buildingClassType).getMaxGlobalInstances()
		...
		if self.top.iActivePlayer != -1:
			if (self.iBuilding == gc.getCivilizationInfo(gc.getPlayer(self.top.iActivePlayer).getCivilizationType()).getCivilizationBuildings(buildingClassType)):
...
# Rise of Mankind end
		...
		for j in range(gc.getNumFeatureInfos()):
			if (gc.getBuildingInfo(self.iBuilding).isPrereqOrFeature(j)):
				if (not bFirst):
					screen.attachLabel(panelName, "", localText.getText("TXT_KEY_OR", ()))
				screen.attachImageButton(panelName, "", gc.getFeatureInfo(j).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_FEATURE, j, -1, False)
				bFirst = False
		#Afforess end
	...
	def placeUpgradesTo(self):
		screen = self.top.getScreen()
		panelName = self.top.getNextWidgetName()
		screen.addPanel(panelName, localText.getText("TXT_KEY_PEDIA_REPLACED_BY", ()), "", False, True, self.X_REPLACED_BY_PANE, self.Y_REPLACED_BY_PANE, self.W_REPLACED_BY_PANE, self.H_REPLACED_BY_PANE, PanelStyles.PANEL_STYLE_BLUE50)
		screen.attachLabel(panelName, "", "  ")
		for j in range(gc.getNumBuildingClassInfos()):
			if (gc.getBuildingInfo(self.iBuilding).isReplaceBuildingClass(j)):
				iBuilding = gc.getBuildingClassInfo(j).getDefaultBuildingIndex()
				if (iBuilding > 0):
					screen.attachImageButton(panelName, "", gc.getBuildingInfo(iBuilding).getButton(), GenericButtonSizes.BUTTON_SIZE_CUSTOM, WidgetTypes.WIDGET_PEDIA_JUMP_TO_BUILDING, iBuilding, -1, False)
	...
	def placeStrategyText(self):
		...
		screen.attachLabel(panelName, "", "  ")
		szStrategyText = gc.getBuildingInfo(self.iBuilding).getStrategy()
		szStrategyTextTextWidget = self.top.getNextWidgetName()
		screen.addMultilineText(szStrategyTextTextWidget, szStrategyText, self.X_STRATEGY_PANE+5, self.Y_STRATEGY_PANE+30, self.W_STRATEGY_PANE - (5 * 2), self.H_STRATEGY_PANE - (15 * 2), WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)

	def getBuildingType(self, iBuilding):
		if gc.getBuildingInfo(iBuilding).getSpecialBuildingType()  == gc.getInfoTypeForString("SPECIALBUILDING_C2C_CULTURE"): ## culture building are Great Wonders
			return 3
