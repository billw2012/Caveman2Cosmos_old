...
#
# Dec 2012 
#  - redid Heroes using The_J's code 
#  - merged Civ IV and BtS concepts into Game Concepts
#  - merged RoM, AND, and C2C concepts into C2C Concepts
#

# Apr 2013
#  - add in a page for Routes - still needsa dll support
#  - split resources into map, culture and manufactured
#  - split the Cultural buildings into a separate building page

# May 2013
# - add in a page for Natural Wonders (a unique terrain feature)
...
import SevoPediaHero
import SevoPediaRoute
import SevoPediaBuilding
import SevoPediaC2CCulture
...
class SevoPediaMain(CvPediaScreen.CvPediaScreen):
	...
	def __init__(self):
		...
		self.mapListGenerators = {
			...
			SevoScreenEnums.PEDIA_PROM_EQUIPIMENT		: self.placeEquipmentPromotions,
			SevoScreenEnums.PEDIA_PROM_AFFLICTIONS		: self.placeAfflictionPromotions,
...
##c2C Cultures - part 1 - Dancing Hoskuld
			SevoScreenEnums.PEDIA_C2C_CULTURES		: self.placeC2CCultures,
##c2C Cultures - part 1 - Dancing Hoskuld
			#Afforess
...
## Natural Wonders Start ##
			SevoScreenEnums.PEDIA_NATURAL_WONDERS	: self.placeNaturalWonders,
## Natural Wonders End ##
##Bonuses - part 1 - Dancing Hoskuld
			##SevoScreenEnums.PEDIA_BONUSES		: self.placeBonuses,
			SevoScreenEnums.PEDIA_BONUSES			: self.placeMapBonuses,
##Bonuses - part 1 - Dancing Hoskuld
...
###world units - part 1 - The_J
			SevoScreenEnums.PEDIA_WORLD_UNITS	: self.placeWorldUnits,
###world units - part 1 - The_J                        
###routes - part 1 - Dancing Hoskuld
			SevoScreenEnums.PEDIA_ROUTES	: self.placeRoutes,
###routes - part 1 - Dancing Hoskuld                        
###Bonuses - part 2 - Dancing Hoskuld
			SevoScreenEnums.PEDIA_BONUSES_MAN	: self.placeManufacturedBonuses,
			SevoScreenEnums.PEDIA_BONUSES_CULT	: self.placeCulturalBonuses,
###Bonuses - part 2 - Dancing Hoskuld                       
		...
		self.mapScreenFunctions = {
			...
			SevoScreenEnums.PEDIA_PROM_EQUIPIMENT		: SevoPediaPromotion.SevoPediaPromotion(self),
			SevoScreenEnums.PEDIA_PROM_AFFLICTIONS		: SevoPediaPromotion.SevoPediaPromotion(self),
...
###world units - part 2 - The_J                        
###Routes - part 2 - Dancing Hoskuld
			SevoScreenEnums.PEDIA_ROUTES	:  SevoPediaRoute.SevoPediaRoute(self),
###Routes - part 2 - Dancing Hoskuld                        
###Bonuses - part 3 - Dancing Hoskuld
			SevoScreenEnums.PEDIA_BONUSES_MAN		: SevoPediaBonus.SevoPediaBonus(self),
			SevoScreenEnums.PEDIA_BONUSES_CULT		: SevoPediaBonus.SevoPediaBonus(self),
###Bonuses - part 3 - Dancing Hoskuld                        
###C2C Cultures - part 2 - Dancing Hoskuld
			SevoScreenEnums.PEDIA_C2C_CULTURES		: SevoPediaC2CCulture.SevoPediaC2CCulture(self),
###C2C Cultures - part 2 - Dancing Hoskuld
## Natural Wonders Start ##
			SevoScreenEnums.PEDIA_NATURAL_WONDERS	: SevoPediaFeature.SevoPediaFeature(self),
## Natural Wonders End ##
	...
	def pediaJump(self, iCategory, iItem, bRemoveFwdList, bIsLink):
		...
		if (iCategory == SevoScreenEnums.PEDIA_MAIN):
			...
			return
###world units - part 3 - The_J
		if (iCategory == SevoScreenEnums.PEDIA_UNITS):                       
			cvUnit = gc.getUnitInfo(iItem)
			cvUnitClass = gc.getUnitClassInfo(cvUnit.getUnitClassType ())
			if cvUnitClass.getMaxGlobalInstances ()==1:
				iCategory = iCategory+1
###world units - part 3 - The_J
###Bonuses - part 4 - Dancing Hoskuld
		if (iCategory == SevoScreenEnums.PEDIA_BONUSES):                       
			cvBonus = gc.getBonusInfo(iItem)
			cvBonusApp = gc.getBonusInfo(iItem).getConstAppearance ()
			if cvBonusApp==0: ## not a map resource
				iCategory = iCategory+1
				cvBonusType = gc.getBonusInfo(iItem).getBonusClassType()
				if cvBonusType == gc.getInfoTypeForString("BONUSCLASS_CULTURE"): ## culture resource
					iCategory = iCategory+1
###Bonuses - part 4 - Dancing Hoskuld
## Natural Wonders Start ##
		if (iCategory == SevoScreenEnums.PEDIA_FEATURES):
			if gc.getFeatureInfo(iItem).getType().find("_PLATY_") > -1:
				iCategory += 1
## Natural Wonders End ##
		if (iCategory == SevoScreenEnums.PEDIA_PROMOTIONS):
			if gc.getPromotionInfo(iItem).isEquipment():
				iCategory += 1
			elif gc.getPromotionInfo(iItem).isAffliction():
				iCategory += 2
...
###world units - part 4			
		...
		self.szCategoryEquipmentPromotions	= localText.getText("TXT_KEY_PEDIA_CATEGORY_EQUIPMENT_PROMOTION", ())
		self.szCategoryAfflictionPromotions	= localText.getText("TXT_KEY_PEDIA_CATEGORY_AFFLICTION_PROMOTION", ())
...
###C2C Cultures - part 4
		self.szCategoryC2CCutures	= localText.getText("TXT_KEY_PEDIA_CATEGORY_C2C_CULTURES", ())
###C2C Cultures - part 4			
		#Afforess
...
## Natural Wonders Start ##
		self.szCategoryNaturalWonders	= localText.getText("TXT_KEY_NATURAL_WONDERS", ())
## Natural Wonders End ##
###Bonuses - part 5
		#~ self.szCategoryBonuses		= localText.getText("TXT_KEY_PEDIA_CATEGORY_BONUS", ())
		self.szCategoryBonusesMap		= localText.getText("TXT_KEY_PEDIA_CATEGORY_BONUS_MAP", ())
		self.szCategoryBonusesMan   	= localText.getText("TXT_KEY_PEDIA_CATEGORY_BONUS_MANDFACTURED", ())
		self.szCategoryBonusesCult   	= localText.getText("TXT_KEY_PEDIA_CATEGORY_BONUS_CULTURE", ())
###Bonuses - part 5			
		self.szCategoryImprovements	= localText.getText("TXT_KEY_PEDIA_CATEGORY_IMPROVEMENT", ())
###Routes - part 4
		self.szCategoryRoutes   	= localText.getText("TXT_KEY_PEDIA_CATEGORY_ROUTES", ())
###Routes - part 4			
...
###world units - part 5 - The_J                        
			["UNITS",	self.szCategoryWorldUnits],
###world units - part 5 - The_J                           
			["UNITS",	self.szCategoryUnitUpgrades],
			["UNITS",	self.szCategoryUnitCategories],
			["PROMOTIONS",	self.szCategoryPromotions],
			["PROMOTIONS",	self.szCategoryEquipmentPromotions],
			["PROMOTIONS",	self.szCategoryAfflictionPromotions],
...
###C2C Cultures - part 6
			["BUILDINGS",	self.szCategoryC2CCutures],
###C2C Cultures - part 6
			#Afforess
...
## Natural Wonders End ##  
###Bonuses - part 6
			["TERRAINS",	self.szCategoryBonusesMap],
			["TERRAINS",	self.szCategoryBonusesMan],
			["TERRAINS",	self.szCategoryBonusesCult],
###Bonuses - part 6
			["TERRAINS",	self.szCategoryImprovements],
###Routes - part 5 - Dancing Hoskuld                      
			["ROUTES",	self.szCategoryRoutes],
###Routes - part 5 - Dancing Hoskuld                           
		...
		self.categoryGraphics = {
			...
			"ROUTES"	: u"%c  " %(CyGame().getSymbolID(FontSymbols.MOVES_CHAR)),
	...
	def placeUnits(self):
	###old code start - The_J
		#self.list = self.getUnitList()
		#self.placeItems(WidgetTypes.WIDGET_PEDIA_JUMP_TO_UNIT, gc.getUnitInfo)
	###old code end - The_J
###world units part 6 - The_J
		self.list = self.getSortedUnitList(gc.getNumUnitInfos(), gc.getUnitInfo,False)
		self.placeItems(WidgetTypes.WIDGET_PEDIA_JUMP_TO_UNIT, gc.getUnitInfo)
###world units part 6  - The_J

###world units part 7 - The_J
	def placeWorldUnits(self):

		self.list = self.getSortedUnitList(gc.getNumUnitInfos(), gc.getUnitInfo,True)
		self.placeItems(WidgetTypes.WIDGET_PEDIA_JUMP_TO_UNIT, gc.getUnitInfo)
###world units part 7 - The_J		
	...
	def getPromotionList(self, iType):
		...
		for i in range(gc.getNumPromotionInfos()):
			...
			cvPromotion = gc.getPromotionInfo(i)
			if (iType == 0) and not cvPromotion.isEquipment() and not cvPromotion.isAffliction():
				list1.append(i)
				iNumInfos += 1
			elif (iType == 1) and cvPromotion.isEquipment():
				list1.append(i)
				iNumInfos += 1
			elif iType == 2 and cvPromotion.isAffliction():
				list1.append(i)
				iNumInfos += 1

		list2 = [(0,0)] * iNumInfos
		i = 0
		for iPromotion in list1:
			list2[i] = (gc.getPromotionInfo(iPromotion).getDescription(), iPromotion)
			i += 1                                
...
###C2C Cultures part 7 - Dancing Hoskuld
	def placeC2CCultures(self):
		self.list = self.getC2CCulturesList()
		self.placeItems(WidgetTypes.WIDGET_PEDIA_JUMP_TO_BUILDING, gc.getBuildingInfo)
	
	def getC2CCulturesList(self):
		return self.pediaBuilding.getBuildingSortedList(3)
###C2C Cultures part 7 - Dancing Hoskuld
...
##Bonuses - part ? Dancing Hoskuld
	def placeMapBonuses(self):
		...
		self.list = self.getSortedBonusList(gc.getNumBonusInfos(), gc.getBonusInfo,0)
		self.placeItems(WidgetTypes.WIDGET_PEDIA_JUMP_TO_BONUS, gc.getBonusInfo)
		
	def placeManufacturedBonuses(self):
		self.list = self.getSortedBonusList(gc.getNumBonusInfos(), gc.getBonusInfo,1)
		self.placeItems(WidgetTypes.WIDGET_PEDIA_JUMP_TO_BONUS, gc.getBonusInfo)
		
	def placeCulturalBonuses(self):
		self.list = self.getSortedBonusList(gc.getNumBonusInfos(), gc.getBonusInfo,2)
		self.placeItems(WidgetTypes.WIDGET_PEDIA_JUMP_TO_BONUS, gc.getBonusInfo)
...
#		return info.getType().find("_TRAIT_") != -1
	...
	def link(self, szLink):
		...
		#Afforess
			...
			return self.pediaJump(SevoScreenEnums.PEDIA_MAIN, SevoScreenEnums.PEDIA_ROUTES, True, True)
...
###Bonuses - part 8 - Dancing Hoskuld
	def getSortedBonusList(self, numInfos, getInfo,iType):
		list1 = []
		iNumInfos = 0
		iBonusClassIsCulture = gc.getInfoTypeForString("BONUSCLASS_CULTURE")
		for i in range(numInfos):
			
			cvBonus = gc.getBonusInfo(i)
			cvBonusApp = gc.getBonusInfo(i).getConstAppearance ()
			if (not cvBonusApp==0) and iType == 0: ## a map resource
				list1.append(i)
				iNumInfos += 1
			elif cvBonusApp==0:
				iBonusTypeClass = gc.getBonusInfo(i).getBonusClassType()
				if iType == 1 and not (iBonusTypeClass == iBonusClassIsCulture): ## manufactured resource 
					list1.append(i)
					iNumInfos += 1
				elif iType == 2 and  iBonusTypeClass == iBonusClassIsCulture: ## culture resource
					list1.append(i)
					iNumInfos += 1
		...
		i = 0
		for iBonus in list1:
			list2[i] = (gc.getBonusInfo(iBonus).getDescription(), iBonus)
			i += 1                                
...
## Natural Wonders Start ##
	def getFeatureSortedList(self, bNaturalWonder):
		listFeatures = []
		iCount = 0
		for iFeature in xrange(gc.getNumFeatureInfos()):
			if not gc.getFeatureInfo(iFeature).isGraphicalOnly():
				if (gc.getFeatureInfo(iFeature).getType().find("_PLATY_") > -1) == bNaturalWonder:
		...
		for iFeature in listFeatures:
			listSorted[iI] = (gc.getFeatureInfo(iFeature).getDescription(), iFeature)
			iI += 1
		listSorted.sort()
		return listSorted
## Natural Wonders End ##
