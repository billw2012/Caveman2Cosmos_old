...
class SevoPediaIndex:
	...
	def buildIndex(self):
		...
		promotionList = self.top.getPromotionList(0)
		p_equipmentList = self.top.getPromotionList(1)
		p_afflictionList = self.top.getPromotionList(2)
		...
		culturalWonderList = self.top.getC2CCulturesList()
		...
		for item in p_equipmentList:
			if (item[0][:18]=="TXT_KEY_PROMOTION_"):
				list.append([item[0][18:].capitalize(),"Promo",item])
			else:
				list.append([item[0],"Promo",item])
		for item in p_afflictionList:
			if (item[0][:18]=="TXT_KEY_PROMOTION_"):
				list.append([item[0][18:].capitalize(),"Promo",item])
			else:
				list.append([item[0],"Promo",item])
		...
		for item in greatWonderList:
			if (item[0][0:4]=="The "):
				...
				list.append([item[0],"Wonder",item])
		for item in culturalWonderList:
			if (item[0][:17]=="TXT_KEY_BUILDING_"):
				list.append([item[0][17:].capitalize(),"Building",item])
			else:
				list.append([item[0],"Building",item])
		...
		for item in leaderList:
			list.append([item[0].capitalize(),"Leader",item])
		for item in traitList:
			list.append([item[0],"Trait",item])
	...
	def placeIndex(self):
		...
		for name, type, item in self.index:
			...
			if (type == "Tech"):
				...
				screen.setTableText(self.tableName, iColumn, iRow, u"<font=3>%c %s</font>" % (CONCEPT_CHAR, item[0]), gc.getNewConceptInfo(item[1]).getButton(), WidgetTypes.WIDGET_PEDIA_DESCRIPTION, CivilopediaPageTypes.CIVILOPEDIA_PAGE_CONCEPT_NEW, item[1], CvUtil.FONT_LEFT_JUSTIFY)
