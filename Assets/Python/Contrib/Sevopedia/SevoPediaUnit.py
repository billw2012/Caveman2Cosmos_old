...
class SevoPediaUnit:
	...
	def __init__(self, main):
		...
		self.H_PROMO_PANE = self.H_SPECIAL_PANE/2 - 5

		self.X_SUBCOMBAT_PANE = self.X_SPECIAL_PANE + self.W_SPECIAL_PANE + 10
		self.Y_SUBCOMBAT_PANE = self.Y_PROMO_PANE + self.H_PROMO_PANE + 10
		self.W_SUBCOMBAT_PANE = self.W_UPGRADES_TO_PANE
		self.H_SUBCOMBAT_PANE = self.H_PROMO_PANE
	...
	def placeStats(self):
		...
		#Dancing Hoskuld ~ if worker unit shrink the text and display the work rate
		iFontSize = 4
		...
		screen.appendListBoxStringNoUpdate(panelName, u"<font=%d>" % iFontSize + szStrength.upper() + u"%c" % CyGame().getSymbolID(FontSymbols.STRENGTH_CHAR) + u"</font>", WidgetTypes.WIDGET_GENERAL, 0, 0, CvUtil.FONT_LEFT_JUSTIFY)
		szName = self.top.getNextWidgetName()
		...
		if (gc.getUnitInfo(self.iUnit).getAirRange() <= 0):
			screen.appendListBoxStringNoUpdate(panelName, u"<font=%d>" % iFontSize + szMovement.upper() + u"%c" % CyGame().getSymbolID(FontSymbols.MOVES_CHAR) + u"</font>", WidgetTypes.WIDGET_GENERAL, 0, 0, CvUtil.FONT_LEFT_JUSTIFY)
		if (gc.getUnitInfo(self.iUnit).getProductionCost() >= 0 and not gc.getUnitInfo(self.iUnit).isFound()):
			...
			screen.appendListBoxStringNoUpdate(panelName, u"<font=%d>" % iFontSize + szCost.upper() + u"%c" % gc.getYieldInfo(YieldTypes.YIELD_PRODUCTION).getChar() + u"</font>", WidgetTypes.WIDGET_GENERAL, 0, 0, CvUtil.FONT_LEFT_JUSTIFY)
		if (gc.getUnitInfo(self.iUnit).getAirRange() > 0):
			szName = self.top.getNextWidgetName()
		...
		if (iWorkRate > 0):
			szName = self.top.getNextWidgetName()
			szRange = localText.getText("TXT_KEY_PEDIA_WORKRATE", (iWorkRate,))
...
# Rise of Mankind 2.71
	...
	def PlaceSubCombatTypes(self):
		screen = self.top.getScreen()
		panelName = self.top.getNextWidgetName()
		screen.addPanel(panelName, localText.getText("TXT_KEY_PEDIA_CATEGORY_SUBCOMBAT_TYPE", ()), "", True, True, self.X_SUBCOMBAT_PANE, self.Y_SUBCOMBAT_PANE, self.W_SUBCOMBAT_PANE, self.H_SUBCOMBAT_PANE, PanelStyles.PANEL_STYLE_BLUE50)
		rowListName = self.top.getNextWidgetName()
		screen.addMultiListControlGFC(rowListName, "", self.X_SUBCOMBAT_PANE+15, self.Y_SUBCOMBAT_PANE+40, self.W_SUBCOMBAT_PANE-20, self.H_SUBCOMBAT_PANE-40, 1, self.PROMOTION_ICON_SIZE, self.PROMOTION_ICON_SIZE, TableStyles.TABLE_STYLE_STANDARD)
		for k in range(gc.getNumUnitCombatInfos ()):
			if (gc.getUnitInfo(self.iUnit).isSubCombatType(k) and not gc.getUnitCombatInfo (k).isGraphicalOnly()):
				screen.appendMultiListButton(rowListName, gc.getUnitCombatInfo (k).getButton(), 0, WidgetTypes.WIDGET_PEDIA_JUMP_TO_UNIT_COMBAT, k, -1, False)
