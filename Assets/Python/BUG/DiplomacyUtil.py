...
## DH 2016 Removed GameUtil import of isAlwaysWar & isPermanentWarPeace
...
def canContact(playerOrID, toPlayerOrID):
	...
	- <toPlayer> must be alive, not minor, and not a NPC
	...
	if playerID == toPlayerID:
		return False
	if not toPlayer.isAlive() or toPlayer.isNPC() or toPlayer.isMinorCiv():
		return False
	if not PlayerUtil.getPlayerTeam(player).isHasMet(toPlayer.getTeam()):
		return False
	if PlayerUtil.getPlayerTeam(player).isAtWar(toPlayer.getTeam()):
		return False
	return True
