...
class CvCustomizableDomesticAdvisor:
  ...
  def __init__(self):
    ...
    self.COLUMNS_LIST = [
        ...
        ("WORLD_WONDERS",        30,   "int",  CyCity.getNumWorldWonders,      None,         0,                  None,                 None,           "u\"WW\""),
        ("MAX_WORLD_WONDERS",        30,   "int",  CyCity.getMaxNumWorldWonders,      None,         0,                  None,                 None,           "u\"MWW\""),
        ("NATIONAL_WONDERS",        30,   "int",  CyCity.getNumNationalWonders,      None,         0,                  None,                 None,           "u\"NW\""),
        ("MAX_NATIONAL_WONDERS",        30,   "int",  CyCity.getMaxNumNationalWonders,      None,         0,                  None,                 None,           "u\"MNW\""),
      ]
...
# BUG - Production Grouping - end
  ...
  def createDictionaries(self):
    ...
    for i in range(gc.getNumBuildingInfos()):
      ...
      self.COLUMNS_LIST.append((key, 24, "bldg", None, None, 0, self.getBuildingState, i, u"u\"" + desc + u"\""))
    ...
    # Properties
    for i in range(gc.getNumPropertyInfos()):
      info = gc.getPropertyInfo(i)
      desc = u"%c" % info.getChar()
      key = info.getType()

      self.COLUMNS_LIST.append((key, 53, "int", None, None, 0, self.calculateProperty, i, "u\"" + desc + "\""))
      self.COLUMNS_LIST.append((key+"_CHANGE", 53, "int", None, None, 0, self.calculatePropertyChange, i, "u\"d" + desc + "\""))
...
# BUG - Production Grouping - end
  ...
  def getBuildingState(self, city, szKey, arg):
...
#         szEffects += u"%d%s " % (iEffect, self.commerceIcons[eCommerceType])
  ...
  def advise(self, city, szKey, type):
    ...
    if self.calculateNetHappiness(city) > 2 and self.calculateNetHealth(city) > 2:
      for bldg in range(gc.getNumBuildingInfos()):
        if self.canAdviseToConstruct(city, bldg):
          info = gc.getBuildingInfo(bldg)
          cost = float(info.getProductionCost())
          if cost > 0:
            value = info.getFoodKept() / cost
            if value > bestData:
              bestOrder = bldg
              bestData = value
    ...
    if ( code == NotifyCode.NOTIFY_LISTBOX_ITEM_SELECTED ):
      ...
      if(szWidget == self.PAGES_DD_NAME + "0"):
        ...
        if (inputClass.getMouseX() == 0):
          ...
          BugUtil.debug("Bring into view selected City 1")
          city = gc.getPlayer(inputClass.getData1()).getCity(inputClass.getData2())
          CyMap().bringIntoView(city.getX(), city.getY(), True, False, True, False, False)
        else:
          city = self.getCurrentCity()
          if (city):
            #CyInterface().lookAtCityOffset(city.getID())
            BugUtil.debug("Bring into view selected City 2")
            CyMap().bringIntoView(city.getX(), city.getY(), True, False, False, True, True)
...
# BUG - Colony Split - end
  ...
  def save(self, inputClass):
    ...
    if name is None:
      name = BugPath.getSettingsDir() + "/CustomDomAdv/CustomDomAdv.txt"
    if name:
      file = open(name, 'w')
      if file:
