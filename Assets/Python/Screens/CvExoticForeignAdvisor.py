...
# BUG - Glance Tab - end
	...
	def drawActive (self, bInitial):
		...
		for iLoopPlayer in range(gc.getMAX_PLAYERS()):
			if (gc.getPlayer(iLoopPlayer).isAlive() and iLoopPlayer != self.iActiveLeader and not gc.getPlayer(iLoopPlayer).isNPC() and  not gc.getPlayer(iLoopPlayer).isMinorCiv()):
...
#		ExoticForPrint ("Entered drawInfo")
		...
		# Their leaderheads		
		for iLoopPlayer in range(gc.getMAX_PLAYERS()):
			if (gc.getPlayer(iLoopPlayer).isAlive() and iLoopPlayer != self.iActiveLeader and (gc.getTeam(gc.getPlayer(iLoopPlayer).getTeam()).isHasMet(gc.getPlayer(self.iActiveLeader).getTeam()) or gc.getGame().isDebugMode()) and not gc.getPlayer(iLoopPlayer).isNPC() and not gc.getPlayer(iLoopPlayer).isMinorCiv()):
...
# Caveman2Cosmos v27 
# Revised in SVN 10206 by KaTiON
		for headerText in (BugUtil.getPlainText("TXT_KEY_FOREIGN_ADVISOR_ABBR_LEADER"),
						   ...
						   BugUtil.getPlainText("TXT_KEY_CIVICOPTION_ABBR_RULE"),
						   BugUtil.getPlainText("TXT_KEY_CIVICOPTION_ABBR_POWER"),
						   BugUtil.getPlainText("TXT_KEY_CIVICOPTION_ABBR_MILITARY"),
						   ...
						   BugUtil.getPlainText("TXT_KEY_CIVICOPTION_ABBR_CURRENCY"),
						   BugUtil.getPlainText("TXT_KEY_CIVICOPTION_ABBR_LABOR"),
						   BugUtil.getPlainText("TXT_KEY_CIVICOPTION_ABBR_EDUCATION"),
						   BugUtil.getPlainText("TXT_KEY_CIVICOPTION_ABBR_LANGUAGE"),
						   BugUtil.getPlainText("TXT_KEY_CIVICOPTION_ABBR_GARBAGE"),
						   BugUtil.getPlainText("TXT_KEY_CIVICOPTION_ABBR_IMMIGRATION"),
						   BugUtil.getPlainText("TXT_KEY_CIVICOPTION_ABBR_AGRICULTURE"),
						   "",
						   fcHeaderText):
# Caveman2Cosmos v27
...
#		ExoticForPrint ("Entered drawGlance")
	...
	def initializeGlance (self):
		...
		for iLoopPlayer in range(gc.getMAX_PLAYERS()):
			...
			and not gc.getPlayer(iLoopPlayer).isNPC()
...
#				ExoticForPrint ("Player = %d" % iLoopPlayer)
				...
				for nHost in range(gc.getMAX_PLAYERS()):
					...
					or gc.getGame().isDebugMode())
					and not gc.getPlayer(nHost).isNPC()
	...
	def drawGlanceChange (self, bInitial):
		...
		if (bInitial):
			...
			for iLoopPlayer in range(gc.getMAX_PLAYERS()):
				if (gc.getPlayer(iLoopPlayer).isAlive() and (gc.getTeam(gc.getPlayer(iLoopPlayer).getTeam()).isHasMet(gc.getPlayer(self.iActiveLeader).getTeam()) or gc.getGame().isDebugMode()) and (iLoopPlayer == self.iActiveLeader or (not gc.getPlayer(iLoopPlayer).isNPC() and not gc.getPlayer(iLoopPlayer).isMinorCiv())) ):
					self.ltPlayerMet [iLoopPlayer] = True
					for nHost in range(gc.getMAX_PLAYERS()):
						if (gc.getPlayer(nHost).isAlive() and nHost != self.iActiveLeader and (gc.getTeam(gc.getPlayer(nHost).getTeam()).isHasMet(gc.getPlayer(self.iActiveLeader).getTeam()) or gc.getGame().isDebugMode()) and not gc.getPlayer(nHost).isNPC() and not gc.getPlayer(nHost).isMinorCiv()):
	...
	def drawResourceDeals(self, bInitial):
		...
		for iLoopBonus in range(gc.getNumBonusInfos()):
			...
			for iLoopPlayer in range(gc.getMAX_PLAYERS()):
				...
				if ( currentPlayer.isAlive() and not currentPlayer.isNPC()
...
## Platy Bonus Screen ##
		...
		screen.addMultiListControlGFC( self.availableMultiList, ""
									 ...
									 , min(self.SURPLUS_WIDTH - 20, self.RESOURCE_ICON_SIZE * len(listSurplus)), self.RESOURCE_ICON_SIZE *3
									 , 1, self.RESOURCE_ICON_SIZE - 2, self.RESOURCE_ICON_SIZE -2, TableStyles.TABLE_STYLE_EMPTY )

		self.availableTable = self.getNextWidgetName()
		iMaxIndex = (self.SURPLUS_WIDTH - 20) / self.RESOURCE_ICON_SIZE
		...
		if (self.RES_SHOW_SURPLUS_AMOUNT_ON_TOP):
			for iIndex in range(len(listSurplus)):
				...
				iY = self.SURPLUS_CIRCLE_Y + self.RESOURCE_ICON_SIZE * (iIndex / iMaxIndex)
				screen.addDDSGFC( self.availableTable + "Circle" + str(iIndex)
								 , ArtFileMgr.getInterfaceArtInfo("WHITE_CIRCLE_40").getPath()
								 , iX, iY
								 , (self.RESOURCE_ICON_SIZE - 2)/2, (self.RESOURCE_ICON_SIZE - 2)/2, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		
		## Platy Add as many rows as you like ##
		self.availableTable2 = self.getNextWidgetName()
		self.availableTable3 = self.getNextWidgetName()
		self.availableTable4 = self.getNextWidgetName()
		self.availableTable5 = self.getNextWidgetName()

		lTables = [self.availableTable, self.availableTable2, self.availableTable3, self.availableTable4, self.availableTable5]
		for i in xrange(len(lTables)):
			screen.addTableControlGFC(lTables[i], min(iMaxIndex, len(listSurplus))
				    , self.SURPLUS_TABLE_X, self.SURPLUS_TABLE_Y + (self.RESOURCE_ICON_SIZE * i)
							     , len(listSurplus) * self.RESOURCE_ICON_SIZE, self.TABLE_CONTROL_HEIGHT
				    , False, False, (self.RESOURCE_ICON_SIZE - 2)/2, (self.RESOURCE_ICON_SIZE - 2)/2, TableStyles.TABLE_STYLE_EMPTY )
...
#			screen.addCheckBoxGFCAt (self.mainAvailablePanel, "Foo" + str(iIndex), gc.getBonusInfo (listSurplus[iIndex]).getButton(), ArtFileMgr.getInterfaceArtInfo ("BUTTON_HILITE_SQUARE").getPath(), self.X_GLANCE_OFFSET + (self.RESOURCE_ICON_SIZE * iIndex), 10, 32, 32, WidgetTypes.WIDGET_PEDIA_JUMP_TO_BONUS, listSurplus[iIndex], -1, ButtonStyles.BUTTON_STYLE_LABEL, False)
			...
			screen.setTableColumnHeader( lTables[iIndex/iMaxIndex], iIndex % iMaxIndex, u"", self.RESOURCE_ICON_SIZE )
			...
			screen.setTableText( lTables[iIndex/iMaxIndex], iIndex % iMaxIndex, 0, amountStr, "", WidgetTypes.WIDGET_GENERAL, -1, -1, 0 )
## Platy Bonus Screen ##			
...
# 		# Assemble the panel that shows the trade table
		...
		for iLoopPlayer in range(gc.getMAX_PLAYERS()):
			currentPlayer = gc.getPlayer(iLoopPlayer)
			if ( currentPlayer.isAlive() and not currentPlayer.isNPC() and not currentPlayer.isMinorCiv() 
										 and gc.getTeam(currentPlayer.getTeam()).isHasMet(activePlayer.getTeam()) 
										 and iLoopPlayer != self.iActiveLeader ):
				message = ""
