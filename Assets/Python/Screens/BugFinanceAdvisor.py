...
class BugFinanceAdvisor:
	...
	def drawContents(self):
		...
		gold = player.getEffectiveGold()
		goldFromCivs = player.getGoldPerTurn()
		goldPerTurn = player.calculateGoldRate()
		playerGoldModifier = player.getCommerceRateModifier(CommerceTypes.COMMERCE_GOLD)
		playerCapitalGoldModifier = player.getCapitalCommerceRateModifier(CommerceTypes.COMMERCE_GOLD)
		...
		for eBldg in range(gc.getNumBuildingInfos()):
			...
			iGlobalMultiplier = info.getGlobalCommerceModifier(CommerceTypes.COMMERCE_GOLD)
			if iMultiplier != 0 or iGlobalMultiplier != 0:
				multipliers.append([eBldg, iMultiplier, iGlobalMultiplier, 0, 0.0])
		...
		for city in PlayerUtil.playerCities(player):
			if not city.isDisorder():
				...
				fSpecialists += (city.getSpecialistPopulation() + city.getNumGreatPeople()) * player.getSpecialistExtraCommerce(CommerceTypes.COMMERCE_GOLD)
				...
				fBonusGoldModifierEffect += fCityTotal * city.getBonusCommerceRateModifier(CommerceTypes.COMMERCE_GOLD) / 100.0
				
				for entry in multipliers:
					eBldg, iMultiplier, _, _, _ = entry
					iCount = city.getNumActiveBuilding(eBldg)
					if iCount > 0:
						entry[3] += iCount
		...
		for entry in multipliers:
			eBldg, iMultiplier, iGlobalMultiplier, iCount, fGold = entry
			if iCount > 0:
				iTotalMinusTaxes += int(fGold)
				if iGlobalMultiplier != 0:
		...
		for eBldg, iMultiplier, iGlobalMultiplier, iCount, fGold in multipliers:
		...
		if fPlayerGoldModifierEffect != 0.0:
			yLocation += self.Y_SPACING
			screen.setLabel(self.getNextWidgetName(), "Background", u"<font=3>" + localText.getText("TXT_KEY_CONCEPT_PLAYER_GOLD_MODIFIERS", ()) + "</font>", CvUtil.FONT_LEFT_JUSTIFY, self.X_INCOME + self.TEXT_MARGIN, yLocation + self.TEXT_MARGIN, self.Z_CONTROLS + self.DZ, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
			screen.setLabel(self.getNextWidgetName(), "Background", u"<font=3>" + unicode(int(fPlayerGoldModifierEffect)) + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.X_INCOME + self.PANE_WIDTH - self.TEXT_MARGIN, yLocation + self.TEXT_MARGIN, self.Z_CONTROLS + self.DZ, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		
		if fBonusGoldModifierEffect != 0.0:
			yLocation += self.Y_SPACING
			screen.setLabel(self.getNextWidgetName(), "Background", u"<font=3>" + localText.getText("TXT_KEY_CONCEPT_BONUS_GOLD_MODIFIERS", ()) + "</font>", CvUtil.FONT_LEFT_JUSTIFY, self.X_INCOME + self.TEXT_MARGIN, yLocation + self.TEXT_MARGIN, self.Z_CONTROLS + self.DZ, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
			screen.setLabel(self.getNextWidgetName(), "Background", u"<font=3>" + unicode(int(fBonusGoldModifierEffect)) + "</font>", CvUtil.FONT_RIGHT_JUSTIFY, self.X_INCOME + self.PANE_WIDTH - self.TEXT_MARGIN, yLocation + self.TEXT_MARGIN, self.Z_CONTROLS + self.DZ, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
