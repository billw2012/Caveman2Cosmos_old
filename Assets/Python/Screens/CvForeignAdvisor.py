...
class CvForeignAdvisor:
	...
	def drawPossibleDeals(self):
		...
		if (gc.getTeam(playerActive.getTeam()).isGoldTrading() or gc.getTeam(playerSelected.getTeam()).isGoldTrading()):
			if (self.iScreen == FOREIGN_BONUS_SCREEN):
				...
				szPlayerName += u" : " + localText.getText("TXT_KEY_MISC_GOLD", (playerActive.getEffectiveGold(), ))
		...
		if (self.iScreen == FOREIGN_BONUS_SCREEN):
			...
			for iLoopBonus in range(gc.getNumBonusInfos()):
				...
				if (self.iSelectedLeader == self.iActiveLeader):
					...
					for iLoopPlayer in range(gc.getMAX_PLAYERS()):
						if (gc.getPlayer(iLoopPlayer).isAlive() and not gc.getPlayer(iLoopPlayer).isNPC() and not gc.getPlayer(iLoopPlayer).isMinorCiv() and gc.getTeam(gc.getPlayer(iLoopPlayer).getTeam()).isHasMet(gc.getPlayer(self.iActiveLeader).getTeam())):
	...
	def drawRelations(self, bInitial):
		...
		for iPlayer in range(gc.getMAX_PLAYERS()):
			...
			if (player.isAlive() and iPlayer != self.iActiveLeader and (gc.getTeam(player.getTeam()).isHasMet(gc.getPlayer(self.iActiveLeader).getTeam()) or gc.getGame().isDebugMode()) and not player.isNPC() and not player.isMinorCiv()):
		...
		screen.setLabel(self.getNextWidgetName(), "", u"<font=2>" + localText.getText("TXT_KEY_FOREIGN_ADVISOR_CONSTRAINTS", ()) + u"</font>", CvUtil.FONT_LEFT_JUSTIFY, self.X_CONTROL + 5, self.Y_CONTROL + 5, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )

		#Establish initial relations screen control conditions
		if (self.cbToggle % 2 == 0) :
			screen.setText(self.CLOSE_BORDERS_ID , "CB", localText.getText("TXT_KEY_FOREIGN_ADVISOR_BORDER_TENSION_OFF", ()), CvUtil.FONT_LEFT_JUSTIFY, self.CONTROL_AREA_X, self.CONTROL_AREA_Y + 0, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, 0, 0)
		else :
			screen.setText(self.CLOSE_BORDERS_ID , "CB", localText.getText("TXT_KEY_FOREIGN_ADVISOR_BORDER_TENSION_ON", ()), CvUtil.FONT_LEFT_JUSTIFY, self.CONTROL_AREA_X, self.CONTROL_AREA_Y + 0, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, 0, 0)
		if (self.hrToggle % 2 == 0) :
			screen.setText(self.HEATHEN_RELIGION_ID , "HR", localText.getText("TXT_KEY_FOREIGN_ADVISOR_RELIGIOUS_TENSION_OFF", ()), CvUtil.FONT_LEFT_JUSTIFY, self.CONTROL_AREA_X, self.CONTROL_AREA_Y + self.CONTROL_AREA_SPACING, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, 0, 0)
		else :
			screen.setText(self.HEATHEN_RELIGION_ID , "HR", localText.getText("TXT_KEY_FOREIGN_ADVISOR_RELIGIOUS_TENSION_ON", ()), CvUtil.FONT_LEFT_JUSTIFY, self.CONTROL_AREA_X, self.CONTROL_AREA_Y + self.CONTROL_AREA_SPACING, 0, FontTypes.TITLE_FONT, WidgetTypes.WIDGET_GENERAL, 0, 0)
		#RevolutionDCM end
		...
		for iSelectedLeader in range(gc.getMAX_PLAYERS()):
			bDisplayed = (not gc.getPlayer(iSelectedLeader).isNPC() and not gc.getPlayer(iSelectedLeader).isMinorCiv() and gc.getPlayer(iSelectedLeader).isAlive() and (gc.getGame().isDebugMode() or gc.getTeam(playerActive.getTeam()).isHasMet(gc.getPlayer(iSelectedLeader).getTeam())))
