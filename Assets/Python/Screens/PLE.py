# No changes made for update to 4.4 as C2C version has extra validity tests and turns PLE off if City Screen is up for unit/building filters.
...
# BUG - Stack Promotions - end
	...
	def showPlotListButtonObjects(self, screen):
		if ( PleOpt.isShowButtons() and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_HIDE_ALL and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_MINIMAP_ONLY and CyInterface().getShowInterface() != InterfaceVisibility.INTERFACE_ADVANCED_START and not CyInterface().isCityScreenUp() ):
			# show PLE modes switches
			# AIAndy: Changed to not shown on the city screen
...
#			screen.addStackedBarGFC( szStringHealthBar, x+7, y-7, 25, 14, InfoBarTypes.NUM_INFOBAR_TYPES, WidgetTypes.WIDGET_GENERAL, -1, -1 )
	...
	#		iUpgradePriceGrp 		= mt.getUpgradePrice(pUnit, iUnitType, 1)
		...
		iGold = gc.getActivePlayer().getEffectiveGold()	
