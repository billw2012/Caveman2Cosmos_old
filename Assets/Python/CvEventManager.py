...
#			'playerGoldTrade'			: self.onPlayerGoldTrade,
	...
	def gameStart(self, bNewGame=False):
		...
		self.iGameSpeed = iGameSpeed	= GAME.getGameSpeedType()
		CvGameSpeedInfo = GC.getGameSpeedInfo(iGameSpeed)
		...
		if iCityID in aWonderTuple[3] and iPlayer in aWonderTuple[4]:
			for i, ID in enumerate(aWonderTuple[3]):
				...
				if KEY == "CRUSADE":
					...
					aBiodomeList = self.aBiodomeList
					if not aBiodomeList or GAME.getGameTurn() % (self.iGameSpeed + 1): continue
					CyPlayer = GC.getPlayer(iPlayer)
					X = CyCity.getX()
					Y = CyCity.getY()
					iUnit = aBiodomeList[GAME.getSorenRandNum(len(aBiodomeList), "Which Animal")]
					CyUnit = CyPlayer.initUnit(iUnit, X, Y, UnitAITypes.NO_UNITAI, DirectionTypes.NO_DIRECTION)
					CyCity.addProductionExperience(CyUnit, False)
					if CyPlayer.isHuman():
