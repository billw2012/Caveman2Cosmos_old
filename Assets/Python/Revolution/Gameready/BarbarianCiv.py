...
class BarbarianCiv :
  ...
  def __init__(self, customEM, RevOpt ) :
    ...
    if( not SDTK.sdObjectExists( "BarbarianCiv", game ) ) :
      ...
      print "BarbarianCiv did not exist, added it"
      if SDTK.sdObjectSetVal( "BarbarianCiv", game, "AlwaysMinorList", [] ):
        print "BarbarianCiv: Successfully added AlwaysMinorList"
      else:
        print "BarbarianCiv: Added AlwaysMinorList failed"
    elif( SDTK.sdObjectGetVal( "BarbarianCiv", game, "AlwaysMinorList" ) == None ) :
      SDTK.sdObjectSetVal( "BarbarianCiv", game, "AlwaysMinorList", [] )
      print "BarbarianCiv: AlwaysMinorList did not exist, added it"
...
##********************************
  ...
  def onEndPlayerTurn( self, argsList ) :
    ...
    if( iNextPlayer >= 0 and iNextPlayer <= gc.getMAX_PLAYERS()):
      if( gc.getPlayer(iNextPlayer).isMinorCiv() and not gc.getPlayer(iNextPlayer).isNPC() ) :
...
#           break
  ...
  def pickCivAndLeader( self, cityList, newPlayerIdx, cultPlayer = None, artStyle = None, bVerbose = True ) :
    ...
    availableCivs = list()
    # Civs with similar style to cultPlayer, if they exist - TB NOTE: this could be a rare bug spot I'm not sure how to resolve - iBarbarian in this case should refer to all NPC civilizations.  RevDef.sXMLBarbarian... not sure how to work with how that gets set.
...
#               iPartials -= 1
    ...
    while (iPartials > 0 and iPass < 4) :
      ...
      for techID in range(0,gc.getNumTechInfos()) :
        ...
        iTechPower = tech.getPowerValue(True)
    ...
    if (playerI.isAlive()):
      ...
      if( playerI.isMinorCiv() and playerI.getNumCities() > 0 ) :
        ...
        if( SDTK.sdObjectExists( 'BarbarianCiv', playerI ) and SDTK.sdObjectGetVal( "BarbarianCiv", playerI, "SetupComplete" ) == 0 ) :
          ...
          for idx in range(0,gc.getMAX_PC_TEAMS()) :
          ...
          if( playerI.getCitiesLost() > 0 ) :
            attacker = None
            for jdx in range(0,gc.getMAX_PC_PLAYERS()) :
...
# Rise of Mankind 2.61 end
    ...
    for barbCity in barbCityList :
      ...
      #ls612: Make this algorithm much more aggressive at finding new cities to add to the new civ.
      if( pBarbCity.area().getID() == capital.area().getID() ) :
        if( pBarbCity.getPreviousOwner() == -1 ) :
          ...
          iBarbCityRand = (game.getSorenRandNum(iDist*iDist, "Give barb city") + 1)
          iAllowableDistance = (((gc.getMap().getGridHeight() + gc.getMap().getGridWidth()) / 20) + ((gc.getGame().getElapsedGameTurns() / gc.getGame().getEstimateEndTurn()) * 20))
          ...
          #New Code for C2C
          iCityJoinFactor = ((pBarbCity.getPopulation() * 30) / iBarbCityRand)
          if (iCityJoinFactor >= 2 and iDist2 <= iAllowableDistance) :
            pBarbCityPlot = pBarbCity.plot() 
            if( self.LOG_DEBUG ) : CvUtil.pyPrint("  BC - Adding extra barb city %s at %d, %d"%(pBarbCity.getName(),pBarbCity.getX(),pBarbCity.getY()))
            pBarbCityPlot.setOwner( newPlayer.getID() )
            self.setupFormerBarbCity(pBarbCityPlot.getPlotCity(), newPlayer.getID(), iBestDefender, int(iNumBarbDefenders*self.militaryStrength + 0.5))
...
# Rise of Mankind 2.6 end
        ...
        if( not pPlot == None and not pPlot.isNone() ) :
          if( pPlot.area().getID() == capital.area().getID() ) :
            ...
            if( not pPlotCity == None and not pPlotCity.isNone() ) :
              if( not pPlotCity.getTeam() == newTeam.getID() and not pPlotCity.isNPC() ) :
    ...
    if( style == "Military" and not newPlayer.isMinorCiv() and not newPlayer.isHuman() ) :
      ...
      if( (not iAttackPlayer == None) and iAttackPlayer >= 0 and iAttackPlayer < gc.getMAX_PC_PLAYERS() ) :
    ...
    warString = ""
    for teamIdx in range(0,gc.getMAX_PC_TEAMS()) :
