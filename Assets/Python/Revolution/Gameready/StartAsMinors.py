		...
		if( gc.getGame().isOption(GameOptionTypes.GAMEOPTION_START_AS_MINORS) ) :
			...
			if( game.isFinalInitialized() and game.getGameTurn() < 1 and pPlayer.isMinorCiv() ) :
				if( pPlayer.isAlive() and not pPlayer.isNPC() ) :
					if( pPlayer.getNumUnits() > 0 and not game.isInAdvancedStart() ) :
						...
						if( not startPlot is None and not startPlot.isNone() and pPlayer.getUnitClassCount(warriorClass) < 1 ) :
...
def onTechAcquired(argsList):
	...
	if( (iTeam >= 0) and (iTeam <= gc.getMAX_PC_TEAMS()) ) :
