...
#-------------------------------------------------------------------------------------------------		
	...
	def onGameLoad( self, argsList ) :
		...
		elif( SdToolKit.sdObjectGetVal( "AIAutoPlay", game, "bCanCancelAuto" ) == None ) :
			SdToolKit.sdObjectSetVal( "AIAutoPlay", game, "bCanCancelAuto", True )
		for idx in range(0,gc.getMAX_PC_PLAYERS()) :
...
#-------------------------------------------------------------------------------------------------   
			...
			if( theKey == int(InputTypes.KB_M) and self.customEM.bShift and self.customEM.bCtrl ) :
				# Toggle auto moves
				if( self.LOG_DEBUG ) : CyInterface().addImmediateMessage(localText.getText("TXT_KEY_AIAUTOPLAY_MOVING", ()),"")
...
#-------------------------------------------------------------------------------------------------   
		...
		if( autoIdx == 0 ) :
			if( self.LOG_DEBUG ) : CyInterface().addImmediateMessage(localText.getText("TXT_KEY_AIAUTOPLAY_CLEARING_ALL", ()),"")
			#self.clearAllAutomation( )
		elif( autoIdx == 1 ) :
			if( numTurns > 0 ) :
				if( self.LOG_DEBUG ) : CyInterface().addImmediateMessage(localText.getText("TXT_KEY_AIAUTOPLAY_FULLY_AUTO", ()) %numTurns, "")
#-------------------------------------------------------------------------------------------------
# Lemmy101 RevolutionMP edit
#-------------------------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------------------------
	...
	def abdicate( self, playerID, numTurns = -1, voluntary = False ) :
		...
		#	popup.launch()
		#else :
			if( voluntary ) : SdToolKit.sdObjectSetVal( "AIAutoPlay", game, "bCanCancelAuto", True )
